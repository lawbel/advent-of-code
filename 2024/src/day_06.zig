...
pub fn part1(alloc: std.mem.Allocator, text: []const u8) !u32 {
    ...
    return @intCast(map.visited.size);
}
...
const Coord = struct {
    ...
    /// Add two coordinates together. If non-null `bounds` are given, returns
    /// an error when the result would go beyond those bounds.
    fn add(self: Self, other: Self, bounds: ?Self) !Self {
        // Note: we ought to check for integer overflow, but for
        // simplicity we will not bother with that. It shouldn't be needed
        // for the challenge.
    ...
    /// Subtract one coordinate from another. Returns an error if the result
    /// would go into negative coordinates.
...
const Obstacles = struct {
    ...
    const Index =
        std.AutoArrayHashMapUnmanaged(usize, std.ArrayListUnmanaged(usize));
    const empty: Self =
        .{ .x_then_y = .empty, .y_then_x = .empty };

    /// Is the given coordinate `(x, y)` in this collection of obstacles?
    fn contains(self: Self, coord: Coord) bool {
        const binarySearch = std.sort.binarySearch;
        const ys = self.x_then_y.get(coord.x) orelse return false;
        const index = binarySearch(usize, ys.items, coord.y, order(usize));
        return (index != null);
    }

    /// Add an obstacle to the collection. Not recommended for anything other
    /// than testing, as it is an inefficient way to update this data type.
    fn put(self: *Self, alloc: std.mem.Allocator, coord: Coord) !void {
        const xs_entry =
            try self.y_then_x.getOrPutValue(alloc, coord.y, .empty);
        const xs_list = xs_entry.value_ptr.items;
        const insert_x_at =
            std.sort.lowerBound(usize, xs_list, coord.x, order(usize));
        ...
        const insert_y_at =
            std.sort.lowerBound(usize, ys_list, coord.y, order(usize));
...
const Guard = struct { pos: Coord, facing: Direction };
...
const Map = struct {
    ...
    height: usize,
    obstacles: Obstacles,
    visited: std.AutoHashMapUnmanaged(Coord, void),
    guard: Guard,

    /// Which (ASCII) character is used for obstacles.
    const obstacle: u8 = '#';

    /// Which (ASCII) characters are used for directions. These are expected
    ...
    /// * If the guard would move past the edges of the map, stop them at the
    ///   edge just before leaving the map and return an error.
    ///
    /// Updates the `.visited` map with any new positions visited during
    /// this patrol.
    fn patrol(self: *Self, alloc: std.mem.Allocator) !void {
        const bounds: Coord = .{ .x = self.width, .y = self.height };
        var loops: u8 = 0;
        ...
        while (true) : (loops += 1) {
            ...
            const in_front: Coord = switch (self.guard.facing) {
        ...
        // Now there is no obstacle immediately in front of us, so we can make
        // some progress. Move as far forward as possible without hitting any
        // obstacles, throwing an error if we would move past the edges of
        // the map.
        ...
        // then we would leave the map completely! In that case, move us right
        // up to the edge as far as we can go, and return an error to signal
        // this scenario.
        //
        ...
        switch (self.guard.facing) {
            .North, .South => |facing| {
                ...
                const below = lowerBound(usize, obs_ys, pos_y, order(usize));
                ...
                const stop = if (up) pos_y + 1 else self.guard.pos.y + 1;
                for (start..stop) |y| {
                    const visited: Coord = .{ .x = pos_x, .y = y };
            ...
            .East, .West => |facing| {
                ...
                const after = lowerBound(usize, obs_xs, pos_x, order(usize));
                ...
                const stop = if (right) self.guard.pos.x + 1 else pos_x + 1;
                for (start..stop) |x| {
                    const visited: Coord = .{ .x = x, .y = pos_y };
                    try self.visited.put(alloc, visited, {});
                }
    ...
    /// Parse a map from a string. It is expected that the map will be a grid
    /// (every row has the same length), and there should be exactly one guard
    /// on the map.
    fn parse(alloc: std.mem.Allocator, text: []const u8) !Self {
        ...
        // order obstacles come in, and only sort them once they are full. The
        // lists being temporarily unsorted does violate the expected invariant
        // on this type, so it is important to sort them before returning.
        var obstacles: Obstacles = .empty;
        errdefer obstacles.deinit(alloc);

        // Loop over each line in the input.
        while (lines.next()) |line| : (height += 1) {
            if (width) |w| {
                if (line.len != w) return error.UnevenLineLengths;
            } else {
                width = line.len;
            }
            ...
            for (line, 0..) |char, x| {
                const coord: Coord = .{ .x = x, .y = height };
                ...
                // If it's a guard facing in some direction, make a record of
                // that. Check that there is not more than one guard.
        ...
        for (obstacles.y_then_x.values()) |x_coords| {
            std.sort.pdq(usize, x_coords.items, {}, std.sort.asc(usize));
        }
        for (obstacles.x_then_y.values()) |y_coords| {
            std.sort.pdq(usize, y_coords.items, {}, std.sort.asc(usize));
        }
...
test "Map.parse" {
    ...
    var actual_map = try Map.parse(alloc, example_map);
    ...
    inline for (fields) |field| {
        ...
        for (expected.keys()) |key| {
            ...
            try std.testing.expectEqualSlices(usize, exp.items, act.items);
        }
    }
}
